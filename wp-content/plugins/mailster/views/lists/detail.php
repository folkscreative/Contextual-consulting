...
<table class="form-table">
	...
	<?php 
	/***
	 * Next tr added for Contextual Consulting
	 * v2.4.3.x3 Sep 2020
	 * extended v2.4.3.x4 Oct 2020 for the other folders
	 */
	$list_archive = get_post_meta($id, 'list_archive', true);
	?>
	<tr>
		<th scope="row"><?php esc_html_e( 'Archive List', 'mailster' ) ?></th>
		<td>
			<label><?php esc_html__( 'Hide this list', 'mailster' ); ?></label>
			<select name="mailster_data[parent_id]" id="">
				<option value="0">Normal</option>
				<option value="9999970" <?php selected('9999970', $list->parent_id); ?>>Workshops</option>
				<option value="9999980" <?php selected('9999980', $list->parent_id); ?>>Recordings</option>
				<option value="9999990" <?php selected('9999990', $list->parent_id); ?>>Admin</option>
				<option value="9999999" <?php selected('9999999', $list->parent_id); ?>>Archived</option>
			</select>
		</td>
	</tr>
</table>
...
if ( ! $is_new ) :
	...
	?>
		...
		<div class="activity-wrap">
			<?php 
			// v3.3.2x0 Contextual Consulting
			// following line replaced for improved performance
			// was actually done during 2022 but I must not have updated the version :-(
			// if ( $activities = $this->get_activity( $list->ID ) ) : 
			if ( $activities = cc_mailster_get_list_activity( $list->ID ) ) : 
				?>
