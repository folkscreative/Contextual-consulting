<?php
/**
 * New version of this file for CC (Aug 2025)
 * needed for PHP v8.3
 * copied from Mailster v4.1.12
 * but with a revised search function
 */
...
class MailsterSubscriberQuery {
	...
	public function run( $args = array(), $campaign_id = null ) {
		...
		/**
		 * Mar 2025
		 * The following code attempts to include a search term into the query
		 * However, it hits a memory limit
		 * ChatGPT recommended a more optimised version which is now being used in its place
		 * The first version of this still hit memory issues so the next version now only checks the name and email fields
		 */
		...
		*/
		...
		// the new code from ChatGTP (only checking name and email) ....

		// error_log(print_r($wpdb->queries, true)); // Logs all queries

		if ($this->args['s']) {
		    // Clean and prepare the search term
		    $raw_search = addcslashes(trim($this->args['s']), '%[]_');
		    $search_terms = preg_split('/\s+/', $raw_search); // Split on spaces
		    $search_terms = array_map('trim', $search_terms);
		    $search_terms = array_filter($search_terms);
		    $not_search_terms = array_values(preg_grep('/^-/', $search_terms)); // Negative search terms
		    $search_terms = array_values(array_diff($search_terms, $not_search_terms)); // Normal search terms
		    $not_search_terms = preg_replace('/^-/', '', $not_search_terms);
		    $wildcard = $this->args['strict'] ? '' : '%';

		    // Initialize arrays for search conditions
		    $search_conditions = [];
		    $not_search_conditions = [];

		    // If there are terms to search for, build the search conditions
		    if (!empty($search_terms)) {
		        foreach ($search_terms as $i => $term) {
		            if (empty($term)) {
		                continue;
		            ...
		            // Default to OR but use AND if term starts with "+"
		            $operator = ($i === 0 || strpos($term, '+') === 0) ? 'AND' : 'OR';
		            $term = ltrim($term, '+'); // Remove + if present

		            // Prepare conditions for the term
		            $term_conditions = [];
		            $term = $wildcard . $term . $wildcard; // Apply wildcard if not strict

		            // Search in email and fullname (firstname + lastname)
		            $term_conditions[] = "subscribers.email LIKE '$term'";
		            $term_conditions[] = "CONCAT_WS(' ', field_firstname.meta_value, field_lastname.meta_value) LIKE '$term'";

		            // Append to `$wheres` with the appropriate operator
		            $wheres[] = "$operator (" . implode(" OR ", $term_conditions) . ")";
		        }
		    }

		    // If there are terms to exclude, build the NOT LIKE conditions
		    if (!empty($not_search_terms)) {
		        foreach ($not_search_terms as $term) {
		            if (empty($term)) {
		                continue;
		            }
		            ...
		            // Exclude from email and fullname
		            $not_search_conditions[] = "(subscribers.email NOT LIKE '$term' AND CONCAT_WS(' ', field_firstname.meta_value, field_lastname.meta_value) NOT LIKE '$term')";
		        }

		        if (!empty($not_search_conditions)) {
		            $wheres[] = "AND (" . implode(" AND ", $not_search_conditions) . ")";
		        }
		    }
		}
