		...
		if ( isset( $postdata ) ) {
			...
			if ( $is_autoresponder ) {
				...
				if ( 'mailster_autoresponder_timebased' == $autoresponder['action'] ) {
					...
					/****
					 * v3.3.2x1
					 * following line should only be used where there are multiple hooks ... not even sure it's right in that case but as we don't use multiple hooks ...
					 * 
					$hooks[ $post->ID ] = array_map( 'trim', explode( ',', $autoresponder['hook'] ) );
					 *
					 * replaced by the following lines of code ...
					 ***/
					if(strpos($autoresponder['hook'], ',')){
						// multiple hooks ... use the original Mailster code
						$hooks[ $post->ID ] = array_map( 'trim', explode( ',', $autoresponder['hook'] ) );
					}else{
						// just one hook
						$hooks[ $post->ID ] = trim($autoresponder['hook']);
					}
					// end of v3.3.2x1 change
		...
		if ( 'autoresponder' == $campaign->post_status ) {
			...
			if ( 'mailster_autoresponder_hook' == $autoresponder['action'] ) {
				...
				if(strpos($autoresponder['hook'], ',')){
					...
					// just one hook
					$hooks[ $campaign->ID ] = trim($autoresponder['hook']);
				}
				// end of v3.3.2x1 change
			...
			$mailsterdata['tinymce'] = array(
				'args'   => apply_filters(
					...
					array(
						...
						'directionality'         => 'ltr',
						'fontsize_formats'       => '8px 9px 10px 11px 12px 13px 14px 15px 16px 18px 20px 22px 24px 30px 36px 48px', // Ian Apr 2025 added 9, 11, 13, 15, 16, 20, 22, 30
