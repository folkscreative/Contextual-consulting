...
class MailsterSubscribers {
	...
	public function assign_lists( $subscriber_ids, $lists, $remove_old = false, $added = null ) {
		...
		ccpa_write_log('mailster subscribers class assign_lists');
		$stack = [];
	    foreach ($trace as $index => $call) {
	        $function = $call['function'] ?? 'main';
	        $class = $call['class'] ?? '';
	        $type = $call['type'] ?? '';
		...
		ccpa_write_log($stack);

		ccpa_write_log($subscriber_ids);
		*/
	...
	private function unsubscribe_by_type( $type, $value, $campaign_id = null, $status = null, $index = null ) {
		...
		/***
		 * Contextual Consulting (v2.4.3.x1)
		 * Following section added May 2020
		 * For CC we are only going to unsubscribe people from the newsletter list, regardless of which campaign they are currently receiving
		 */

		// we need the list id for the newsletter list
        ...
        // if list not found, something's gone horribly wrong, let's bale
        if($news_list_id == 0){
        	return false;
        }
        // try to unsubscribe
        if ( $this->unassign_lists( $subscriber->ID, $news_list_id ) ){
        	$status .= '_list';
        	do_action( 'mailster_list_unsubscribe', $subscriber->ID, $campaign_id, $news_list_id, $status );
			if ( $status ) {
	...
	public function get_by_mail( $mail, $custom_fields = false, $include_deleted = false ) {
		 ...
		 * Modified Oct 2025 to include custom fields and include deleted
		 * The get_by_type call is running out of memory if the subscriber is not found
		 * This workaround fixes that
		 */
		return cc_mailster_get_subscriber_by_mail( $mail, $custom_fields, $include_deleted );
