 ...
 * Description: ### Modified by SaaSora for your website - DO NOT UPDATE! ### This is a highly intuitive script that can grab an image by custom field, featured image, post attachment, or extracting it from the post's content.
 * Version:     999.3.1.1
 ...
 * SaaSora changes:
 * v999.3.1.1 Aug 2024
 * - Changed RPM in above description to SaaSora
 * v999.3.1.0 May 2021
 * With PHP v7.3 we are getting errors such as compact(): Undefined variable: post_id in ... get-the-image.php
 * 
 * v999.1.1.0 10/02/2020:
 * Have NOT removed the thumbnail class that is added by default! (see the version in the WMS folder if you want to do that!)
 * Image scan does not work as you might expect!
 * It returns the found URL, not the image size you requested
 * Justin attempted to solve that by adding in a search for id="wp-image-9999"
 * However, that's not what the images look like in the content
 * Therefore, we're upgrading the plugin ...
 * We search the content for the image URL and use that to reverse engineer the ID and then use that to get the correct image size
 * See comments added in the code prefaced by ##### RPM #####
 * NOTE: this needs the Simple Dom Html Parser
 */
...
final class Get_The_Image {
	...
	public function find() {
		...
		// ##### RPM ##### start
		// When using compact before PHP 7.3, any strings that are not set will silently be skipped. From 7.3, they throw an error
		// was ....
		...
		$key = md5( serialize( compact( $safe_array ) ) );
		// ##### RPM ##### end
	...
	public function get_scan_image() {
		...
		// ##### RPM #####
		// The next few lines have been commented out because they do not work
		/*
		...
		*/
		// ##### RPM #####
		// now our replacement code ...
		global $wpdb;
		// we use a dom scanner to find the image source - much simpler, safer and easier to understand than regular expressions :-)
		require_once(dirname(__FILE__).'/simplehtmldom/simple_html_dom.php');
	    $html = str_get_html($post_content);
	    $images = $html->find('img');
	    foreach ($images as $image) {
	    	$image_url = $image->src;
	    	// great, now we have the url ... thing is, this may have the size as a part of it
	    	// so, first, let's see if we can find the source as extracted ...
	    	$attachment = $wpdb->get_col($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE guid='%s';", $image_url ));
	    	if(!empty($attachment)){
	    		$image_id = $attachment[0];
	    		// and having got the id ... let's get the actual image that we want
	    		$this->_get_image_attachment( $image_id );
	    		if ( !empty( $this->image_args ) ){
	    			return;
	    		}
	    	}else{
	    		// that source was not found ... let's try to remove the image size from it and try again ....
	    		$file_type = substr($image_url, strrpos($image_url, '.'));
	    		$shorter_img_url = substr($image_url, 0, strrpos($image_url, '-')).$file_type;
		    	$attachment = $wpdb->get_col($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE guid='%s';", $shorter_img_url ));
		    	if(!empty($attachment)){
		    		$image_id = $attachment[0];
		    		$this->_get_image_attachment( $image_id );
		    		if ( !empty( $this->image_args ) ){
