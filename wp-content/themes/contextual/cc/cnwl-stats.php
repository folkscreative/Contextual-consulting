...
add_action( 'admin_menu', 'cnwl_stats_menu' );
function cnwl_stats_menu() {
...
function cnwl_stats_report(){
	...
	<div class="wrap">
		...
		<div class="text-end">
			<a href="javascript:void(0);" id="cnwl-export-req" class="button button-secondary">Generate CSV export</a><br>
			<span id="cnwl-export-msg"></span>
		</div>
    ...
    $profession   = get_user_meta($user->ID, 'job', true);
							...
							foreach ($metas as $key => $value) {
								if(substr($key, 0, 13) == 'cc_rec_wshop_'){
									...
									if(isset($rec_data['payment_id'])){
										...
										// we now want to show cancelled registrations
										// $payment_data = cc_paymentdb_get_payment($rec_data['payment_id']);
										// if($payment_data['status'] == 'Cancelled') continue;
									}
									$reg_recs[] = array(
										'rec_id' => $rec_id,
										'pmt_id' => $payment_id,
								...
								/* using attendance table instead
							...
							foreach ($wkshop_users as $wkshop_user) {
								...
								/*
								$title = get_the_title($wkshop_user['workshop_id']);
								if(strlen($title) > 50){
									$wkshop_html .= '<span title="'.$title.'">'.substr($title, 0, 50).' ...</span>';
								}else{
									$wkshop_html .= $title;
								}
								*/
								$wkshop_html .= get_the_title($wkshop_user['workshop_id']);
								...
								if( $cancelled ){
									...
									$sum_w_value = $sum_w_value + (float)$pricing['amount'];
								}
							...
							foreach ($reg_recs as $rec_ids) {
								...
								if(strlen($title) > 50){
									$rec_html .= '<span title="'.$title.'">'.substr($title, 0, 50).' ...</span>';
								}else{
									$rec_html .= $title;
								}
								*/
								$rec_html .= get_the_title( $rec_ids['rec_id'] );
								...
								if( $cancelled ){
									...
									$pricing = cc_payment_non_disc_amount( $payment_data, 'GBP' );
									$pricing_html = cc_money_format( $pricing['amount'], $pricing['currency'] );
									$sum_r_value = $sum_r_value + $pricing['amount'];
								}
    ...
    echo '<td colspan="6">&nbsp;</td>';
									...
									if( isset( $workshop_rows[$i] ) ){
										...
										echo '<td class="cnwl-pmt-col '.$td_class.'" valign="top">'.$workshop_rows[$i][1].'</td><td class="'.$td_class.'" valign="top">'.$workshop_rows[$i][2].'</td><td class="truncate '.$td_class.'" valign="top">'.$workshop_rows[$i][3].'</td><td class="'.$td_class.'" valign="top">'.$workshop_rows[$i][6].'</td><td class="'.$td_class.'" valign="top">'.$workshop_rows[$i][4].'</td><td class="'.$td_class.'" valign="top">'.$workshop_rows[$i][5].'</td>';
									}else{
									...
									if( isset( $recording_rows[$i] ) ){
										...
										echo '<td class="cnwl-pmt-col '.$td_class.'" valign="top">'.$recording_rows[$i][1].'</td><td class="'.$td_class.'" valign="top">'.$recording_rows[$i][2].'</td><td class="truncate '.$td_class.'" valign="top">'.$recording_rows[$i][3].'</td><td class="'.$td_class.'" valign="top">'.$recording_rows[$i][5].'</td><td class="'.$td_class.'" valign="top">'.$recording_rows[$i][4].'</td>';
									}else{
					...
					<tfoot>
						<tr>
							<td colspan="2" valign="top">Registered users: <?php echo $count_users; ?></td>
							<td class="cnwl-pmt-col" colspan="3" valign="top">Workshop registrations (excl canc.):</td>
							<td valign="top"><?php echo cc_money_format( $sum_w_value, 'GBP' ); ?></td>
							<td colspan="2" valign="top"><?php echo $count_w_reg; ?></td>
							<td class="cnwl-pmt-col" colspan="3" valign="top">Recording registrations (excl canc.):</td>
							<td valign="top"><?php echo cc_money_format( $sum_r_value, 'GBP' ); ?></td>
							<td valign="top"><?php echo $count_r_reg; ?></td>
						</tr>
						<tr>
							<td colspan="2">&nbsp;</td>
							<td class="cnwl-pmt-col" colspan="4" valign="top">Workshop cancellations (<span class="cancelled">struck-through</span>):</td>
							<td colspan="2" valign="top"><?php echo $count_w_canc; ?></td>
							<td class="cnwl-pmt-col" colspan="4" valign="top">Recording cancellations (<span class="cancelled">struck-through</span>):</td>
							<td valign="top"><?php echo $count_r_canc; ?></td>
						</tr>
						<tr>
							<td colspan="2">&nbsp;</td>
							<td class="cnwl-pmt-col" colspan="4" valign="top">## = Workshop in future/recording viewable:</td>
							<td valign="top"><?php echo $count_w_future; ?></td>
							<td valign="top"><?php echo $count_wr_avail; ?></td>
							<td class="cnwl-pmt-col" colspan="4" valign="top">## = Recording viewable:</td>
							<td valign="top"><?php echo $count_r_avail; ?></td>
						</tr>
						<tr>
							<td colspan="2">&nbsp;</td>
							<td class="cnwl-pmt-col" colspan="4" valign="top">** = Workshop attended live:</td>
							<td valign="top"><?php echo $count_w_view; ?></td>
							<td valign="top">&nbsp;</td>
							<td class="cnwl-pmt-col" colspan="4" valign="top">** = Recording viewed:</td>
							<td valign="top"><?php echo $count_r_view; ?></td>
						</tr>
						<tr>
							<td colspan="2">&nbsp;</td>
							<td class="cnwl-pmt-col" colspan="4" valign="top">$$ = Workshop recording viewed. Count = watched recording only:</td>
							<td valign="top">&nbsp;</td>
							<td valign="top"><?php echo $count_wr_view; ?></td>
							<td class="cnwl-pmt-col" colspan="4" valign="top">X = Did not view in time</td>
							<td valign="top"><?php echo $count_r_dna; ?></td>
						</tr>
						<tr>
							<td colspan="2">&nbsp;</td>
							<td class="cnwl-pmt-col" colspan="4" valign="top"><!-- X = Did not attend/view. 2nd count = DNA training or rec. -->X = Did not attend workshop or view recording</td>
							<td valign="top"><?php // echo $count_w_dna; ?></td>
							<td valign="top"><?php echo $count_w_tot_dna; ?></td>
							<td class="cnwl-pmt-col" colspan="5" valign="top">&nbsp;</td>
						</tr>
						<tr>
							<td colspan="2" valign="top">Registrations by training</td>
							<td class="cnwl-pmt-col" colspan="6" valign="top">
								<?php
								foreach ($count_w_reg_tots as $workshop_id => $count) {
									$title = get_the_title($workshop_id);
									if(strlen($title) > 50){
										$title = '<span title="'.$title.'">'.substr($title, 0, 50).' ...</span>';
									}
									echo $count.': '.$workshop_id.': '.$title.'<br>';
								}
								?>
							</td>
							<td class="cnwl-pmt-col" colspan="5" valign="top">
								<?php
								foreach ($count_r_reg_tots as $recording_id => $count) {
									$title = get_the_title($recording_id);
									if(strlen($title) > 50){
										$title = '<span title="'.$title.'">'.substr($title, 0, 50).' ...</span>';
									}
									echo $count.': '.$recording_id.': '.$title.'<br>';
								}
								?>
							</td>
						</tr>
...
add_action( 'wp_ajax_cnwl_generate_csv', 'cc_cnwl_generate_csv' );
function cc_cnwl_generate_csv(){
	$response = array(
		'status' => 'error',
		'msg' => '',
	);
	$filename = 'CNWL-stats-'.date('Y-m-d-H-i-s').'.csv';
...
if($rec_ids['pmt_id'] > 0){
    $payment_data = cc_paymentdb_get_payment($rec_ids['pmt_id']);
    $reg_html = date('d/m/Y', strtotime($payment_data['last_update']));
}else{
    $reg_html = '';
}
...
add_action('init', 'maybe_download_feedback_csv');

function maybe_download_feedback_csv() {
    ...
    $feedback_rows = $wpdb->get_results($wpdb->prepare("
        ...
        WHERE um.meta_key = 'portal_user' AND um.meta_value = 'cnwl'
