            ...
            <div class="card-body">
                ...
                <form method="get" class="usage-filter">
                    ...
                    <div class="row g-3">
                        ...
                        <!-- Service type -->
                       <div class="col-md-2">';
$html .= '<label class="form-label">Service type</label>';
$html .= '<select class="form-select" name="filter_service_type">';
$html .= '<option value="">All services</option>';

foreach (nlft_service_type() as $slug => $label) {
...
$html .= '</div>
                        <!-- Borough / Care group -->
                        <div class="col-md-2">
                            <label class="form-label">Borough / Care group</label>
                            <select class="form-select" name="filter_borough">
                                <option value="">All boroughs</option>';
foreach (nlft_boroughss() as $slug => $label) {
    $html .= '<option value="' . esc_attr($slug) . '">' . esc_html($label) . '</option>';
}
  $html .= '</select>
                        </div>
        ...
        foreach ($month_rows as $month_data) {
            ...
            <div class="row align-items-center border-bottom py-2">
                ...
                <div class="col-1 text-end">
                    <a href="#" class="dashboard-toggle-details" data-period="' . esc_attr($month_data['key']) . '" data-org="' . esc_attr($portal_user) . '">
 ...
 */
/* function dashboard_get_month_stats_filtered($org, $year, $month, $filter_start, $filter_end) {
    ...
    $query = "
        ...
        WHERE last_update BETWEEN %s AND %s
            AND DISC_CODE = %s
            AND (status = 'Payment not needed'
                OR status = 'Cancelled'
                OR status LIKE 'Linked to #%%')
            AND (type = 'recording' OR type = '' OR type IS NULL)
    ";

    $result = $wpdb->get_row(
        $wpdb->prepare($query, $range_start, $range_end, $org_uc),
        ARRAY_A
    );
                ...
                <form method="get" class="usage-filter">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-2">
                            ...
                            <select class="form-select" name="filter_role">
                                <option value="">All roles</option>
                                <option value="clinician" '.selected($filter_role,'clinician',false).'>Clinician</option>
                                <option value="admin" '.selected($filter_role,'admin',false).'>Admin</option>
                                <option value="manager" '.selected($filter_role,'manager',false).'>Manager</option>
                                <option value="support" '.selected($filter_role,'support',false).'>Support staff</option>
                            </select>
                        </div>
