...
jQuery(document).ready(function( $ ) {
    ...
    $(document).on('click', '.copy-link', function(){
        ...
        $(this).after('<span class="doneit"><i class="fa fa-check" aria-hidden="true"></i> copied to clipboard</span>');
    });
    */

    $(document).on('click', ".copy-link", function() {
        var $button = $(this);
        var inputId = $button.data("link");
        var inputValue = $("#" + inputId).val();
        // Create a temporary textarea to copy the text
        var tempTextarea = $("<textarea>");
        $("body").append(tempTextarea);
        tempTextarea.val(inputValue).select();
        document.execCommand("copy");
        tempTextarea.remove();
    ...
    */
    ...
    $(document).on('click', '.toggler, .workshop-title', function(){
        var parentDiv = $(this).closest('.workshop');
        if(parentDiv.hasClass('opened')){
            parentDiv.removeClass('opened').addClass('closed');
        }else{
            $('.workshop').not(parentDiv).removeClass('opened').addClass('closed');
