<?php
/**
 * Template Name: CE Credits Certificate PDF
 * Generate the PDF version of a CE Credits certificate
 * This is the APA certificate
 *
 * Uses TCPDF
 *
 */
...
if(isset($_GET['c']) && $_GET['c'] <> ''){
	$cert_parms = cc_ce_credits_cert_parms_decode($_GET['c']);
	if($cert_parms){

		cc_debug_log_anything('CE Cert generated for user: '.$cert_parms['user_id'].' for training_id: '.$cert_parms['training_id'].' event_id: '.$cert_parms['event_id']);
		// and also put it into the new CE Cert log table
		cc_cert_log_request( $cert_parms['user_id'], $cert_parms['training_id'], $cert_parms['event_id'] );
		...
		$num_credits = cc_ce_credits_number_credits($cert_parms['training_id'], $cert_parms['event_id']);
		$the_title = html_entity_decode(get_the_title($cert_parms['training_id']));

		if(course_training_type($cert_parms['training_id']) == 'workshop'){
			$pretty_dates = workshop_calculated_prettydates($cert_parms['training_id']);
			$date_text = 'hosted on '.$pretty_dates['london_date'];
		}else{
			$workshop_dates = get_post_meta($cert_parms['training_id'], 'workshop_dates', true);
			if($workshop_dates <> ''){
		...
		if($type == 'recording'){
			...
			if(isset($recording_meta['first_viewed']) && $recording_meta['first_viewed'] <> ''){
				$attendance_txt = 'Attended a recording of the above '.$hrs_text.'workshop on '.date('jS M Y', strtotime($recording_meta['first_viewed']));
			}else{
		...
		*/
		...
		$presenters = cc_presenters_names($cert_parms['training_id'], 'none');
		if($presenters <> ''){
			$pdf->SetFont('helvetica', 'B', 20, '', true);
